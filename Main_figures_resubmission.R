...
colnames(Flurry_full) = c("Protein", "Start", "Peptide", "Haplotype", "Rank","1-log50k", "Binding_affinity", "Processing_score", "Presentation_score")
Flurry_full$Protein[which(Flurry_full$Protein == "orflab")] = "orf1ab"
Flurry_full = Flurry_full[order(Flurry_full$Peptide),]
...
Netii_3_full$Protein[which(Netii_3_full$Protein =="orflab")] = "orf1ab"
Netii_3_full$Haplotype = str_replace_all(Netii_3_full$Haplotype, pattern = "DRB1_", replacement = "HLA-DRB1*")
Netii_3_full$Haplotype = str_replace_all(Netii_3_full$Haplotype, pattern = "HLA-DQA1", replacement = "HLA-DQA1*")
Netii_3_full$Haplotype = str_replace_all(Netii_3_full$Haplotype, pattern = "-DQB1", replacement = "/DQB1*")
Netii_3_full$Haplotype = paste0(substr(Netii_3_full$Haplotype, 1, nchar(Netii_3_full$Haplotype)-2),":", substr(Netii_3_full$Haplotype, nchar(Netii_3_full$Haplotype)-1, nchar(Netii_3_full$Haplotype)))
Netii_3_full$Haplotype[which(substr(Netii_3_full$Haplotype,1,6) == "HLA-DQ")] = paste0(substr(Netii_3_full$Haplotype[which(substr(Netii_3_full$Haplotype,1,6) == "HLA-DQ")] ,1,11),":",
                                                                                       substr(Netii_3_full$Haplotype[which(substr(Netii_3_full$Haplotype,1,6) == "HLA-DQ")] ,12,
...
Netii_4_full$Protein[which(Netii_4_full$Protein =="orflab")] = "orf1ab"
Netii_4_full$Haplotype = str_replace_all(Netii_4_full$Haplotype, pattern = "DRB1_", replacement = "HLA-DRB1*")
Netii_4_full$Haplotype = str_replace_all(Netii_4_full$Haplotype, pattern = "HLA-DQA1", replacement = "HLA-DQA1*")
Netii_4_full$Haplotype = str_replace_all(Netii_4_full$Haplotype, pattern = "-DQB1", replacement = "/DQB1*")
Netii_4_full$Haplotype = paste0(substr(Netii_4_full$Haplotype, 1, nchar(Netii_4_full$Haplotype)-2),":", substr(Netii_4_full$Haplotype, nchar(Netii_4_full$Haplotype)-1, nchar(Netii_4_full$Haplotype)))
Netii_4_full$Haplotype[which(substr(Netii_4_full$Haplotype,1,6) == "HLA-DQ")] = paste0(substr(Netii_4_full$Haplotype[which(substr(Netii_4_full$Haplotype,1,6) == "HLA-DQ")] ,1,11),":",
                                                                         substr(Netii_4_full$Haplotype[which(substr(Netii_4_full$Haplotype,1,6) == "HLA-DQ")] ,12,
...
p=ggplot(data=iedb_comp, aes(x=NetMHCpan_BA_BA, y=affinity, color = allele))+
  ...
  annotate("text",x=2.5, y = cutpoint, label = paste0(cutpoint,"nM"), color="black", size=10,hjust=0, vjust=0)+
  annotate("text",x=HLAI_nM_cut, y = .1, label = paste0(HLAI_nM_cut,"nM"), color="black", size=10, angle=90, hjust=0, vjust=0)+
  theme(
...
#Rename HLA levels to remove "HLA-"
Net_BA$Haplotype = plyr::mapvalues(Net_BA$Haplotype, from = levels(Net_BA$Haplotype), to = str_remove_all(levels(Net_BA$Haplotype), "HLA-"))
...
Plot_2A = plot_grid(
  plot_grid(
    hist_top + theme(legend.position = "none")
...
#Create subset for scatterplot so DRB alleles do not overcrowd.  Set max value to 50

iedb_comp2_sub = data.table()
for(z in levels(iedb_comp2$allele)){
  print(z)
  temp_iedb = iedb_comp2[which(iedb_comp2$allele == z)]
  if((nrow(temp_iedb) > 50) & (substr(z,1,6) == "HLA-DR")){
    temp_iedb = temp_iedb[sample(0:nrow(temp_iedb), size = 50, replace = F),]       
  }
  iedb_comp2_sub = rbind(iedb_comp2_sub, temp_iedb)
}

#Scatterplot showing IEDB viral read nM and netMHCIIpan 3.2 BA
p2=ggplot(data=iedb_comp2_sub, aes(x=NetMHCIIpan_BA, y=affinity, color = allele))+
  geom_point()+
  ...
  annotate("text",x=5, y = cutpoint, label = paste0(cutpoint,"nM"), color="black", size=10, hjust=0, vjust = 0)+
  annotate("text",x=HLAII_nM_cut, y = .1, label = paste0(HLAII_nM_cut,"nM"), color="black", size=10, angle=90, hjust=0, vjust=0)+
  theme(
    legend.text = element_text(color = "white"),
    legend.title = element_text(color = "white"),
    legend.key = element_rect(fill = "white")
...
#Change alleles to remove "HLA"

Netii_SARS$Haplotype = plyr::mapvalues(Netii_SARS$Haplotype, from = levels(Netii_SARS$Haplotype), to = str_remove_all(levels(Netii_SARS$Haplotype), "HLA-"))
...
Plot_2B = plot_grid(
  plot_grid(
    hist_top2 + theme(legend.position = "none")
    , p2 + theme(legend.position = "none")
    , ncol = 1
    , align = "hv")
  , plot_grid(
    get_legend(hist_top2)
    , get_legend(p2)
    , ncol =1)
  , rel_widths = c(7,3)
)

#grid.arrange(Plot_2A, Plot_2B, nrow=1,  ncol=2)
...
#See if Tc epitope from literature is present in our predicted nested epitope set
...
#Plotting Figure 2C
Min_frequency = 45 #Minimum population frequency to show on plot
...
#Grab minimum pMHC nM for each peptide
Min_I = c()
Min_II = c()
for(n in 1:nrow(Net_BA_cast)){
  Min_I = c(Min_I, min(Net_BA_cast[n,4:19]))
}
for(n in 1:nrow(Netii_BA_cast)){
  Min_I = c(Min_I, min(Netii_BA_cast[n,4:18]))
}

Net_all = rbind(Net_BA_cast[,c(1,2,3,21)], Netii_BA_cast[,c(1,2,3,20)])
Net_all$Min_BA = c(Min_I, Min_II)
Net_all$HLA = "HLA-I"
Net_all$HLA[which(nchar(Net_all$Peptide) == 15)] = "HLA-II"
Net_all$Frequency[which(Net_all$HLA == "HLA-II")] = -1*Net_all$Frequency[which(Net_all$HLA == "HLA-II")] 
Net_all$Frequency = Net_all$Frequency*100
Net_all$Murine = "None"
Net_all$Murine[which(Net_all$Peptide %in% mouse1$Peptide)] = "MHC-I"
Net_all$Murine[which(Net_all$Peptide %in% mouse2$Peptide)] = "MHC-II"
...
#N: 9245 - 9663                                                                       
#ORF10: 9664 - 9701
prot_tab = data.table(Protein = c("ORF1ab", "S", "ORF3a", "E", "M", "ORF6", "ORF7a", "ORF8", "N", "ORF10"), 
                      Start = c(1,7097,8370,8645,8720,8942,9003,9124,9245,9664), 
                      End = c(7096,8369,8644,8719,8941,9002,9123,9244,9663,9701))
prot_tab$Protein = factor(prot_tab$Protein, levels = prot_tab$Protein)

p=ggplot(data=Net_all[which(abs(Net_all$Frequency)>Min_frequency)]) + 
  geom_segment(data=Net_all[which(abs(Net_all$Frequency)>Min_frequency)], aes(x=Start, xend=Start, y=0, yend=Frequency),
               color="grey", alpha=0.5, size=.5) +
  geom_point(aes(x=Start, y=Frequency, color=HLA, fill = Murine, size = Min_BA, alpha = abs(Frequency)), shape = 21) +
  scale_fill_manual(values = c("red", "blue", "grey"), name="Murine overlap")+
  scale_color_manual(values = c("red", "blue"), name="HLA", guide = F) +
  scale_y_continuous(name = "Population frequency", labels = abs, breaks = seq(-100,100,20))+
  scale_x_continuous(name = "", limits = c(0,9701), breaks = c(seq(0,9700,1000),9701))+
  scale_size(name = "Min. binding affinity (nM)", range = c(0,15), breaks = c(5,20,80,320), trans = "reciprocal")+ 
  scale_alpha_continuous(guide = F) +
  new_scale("fill") +
  new_scale("color") +
  
  geom_rect(data = prot_tab, aes(xmin=Start, xmax=End, fill = Protein), ymin=-10, ymax=10, color="black", size=0.1) +
  scale_fill_viridis_d(name = "Protein") +
  #geom_text(aes(x=7095/2, y=(y_ll-5), label="orf1ab", angle=0), size=5, color = "black") +
  geom_text(aes(x=1000, y=(10), label="-1000", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=2000, y=(10), label="-2000", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=3000, y=(10), label="-3000", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=4000, y=(10), label="-4000", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=5000, y=(10), label="-5000", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=6000, y=(10), label="-6000", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=7000, y=(10), label="-7000", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  
  #geom_text(aes(x=7096+(8368-7096)/2, y=(y_ll-5), label="S", angle=0), size=5, color = "black") + 
  geom_text(aes(x=7096+200, y=(10), label="-200", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=7096+500, y=(10), label="-500", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=7096+800, y=(10), label="-800", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=7096+1100, y=(10), label="-1100", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  
  #geom_text(aes(x=8368+(8644-8368)/2, y=(y_ll-5), label="ORF3a", angle=0), size=2.5, color = "black") +
  geom_text(aes(x=8369+100, y=(10), label="-100", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=8369+200, y=(10), label="-200", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  
  geom_text(aes(x=9244+100, y=(10), label="-100", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=9244+200, y=(10), label="-200", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=9244+300, y=(10), label="-300", angle=-90), size=4.5, color = "white",  fontface = "plain", hjust=0) +
  
  geom_rect(aes(xmin=9663, xmax=9701, ymin=(-10), ymax=(10)), fill=viridis(10)[10], color="black", size=0.1) +
  theme(text=element_text(face="bold",size=20,colour="black"),
...
#   labs(y="Nested HLA ligand pop. frequency")+
#   theme(text=element_text(face="bold",size=20,colour="black"),
#         axis.title.x=element_blank(),
#         axis.text.x=element_blank(),
#         axis.ticks.x=element_blank()) 
...
sm= ggplot() +
  ...
  geom_rect(data = prot_tab, aes(xmin=Start, xmax=End, fill = Protein), ymin=0, ymax=12, color="black", size=0.1, alpha = 0.25, show.legend = F) +
  scale_fill_viridis_d(name = "Protein") +
  
  new_scale("fill") +
  new_scale("color") +
  
  geom_smooth(data=coverage, aes(x = Position, y=Coverage, color = Class), method = 'loess',span=.1)+
...
text(0.825,.5,paste0("All HLA-I ligands\n",nrow(Net_BA_cast)))
text(0.425,.5,paste0("Nested HLA-I ligands\n", length(unique(Net_coepitopes$c1_peptide))))
...
text(0.825,.5,paste0("All HLA-II ligands\n",nrow(Netii_BA_cast)))
text(0.425,.5,paste0("Nested HLA-II ligands\n",length(unique(Net_coepitopes$c2_peptide))))
...
text(0.75,.55,paste0("Nested HLA ligands: ", nrow(Net_coepitopes)))
...
####(New!) Figure 2G - Curve for pop freq distribtion#####
##########################################################
...
for(n in seq(0,100,.01)){
  print(n)
  hlai_dist = c(hlai_dist, length(which((Net_BA_cast$Frequency*100) > n)))
  hlaii_dist = c(hlaii_dist, length(which((Netii_BA_cast$Frequency*100) > n)))
  nested_dist = c(nested_dist, length(which((Net_coepitopes$Total_freq) > n)))
    
}
dist_table = data.table(rbind(data.table(seq(0,100,.01), hlai_dist, "HLA-I"),
                              data.table(seq(0,100,.01),hlaii_dist, "HLA-II"),use.names=F))
                              #data.table(seq(0,100,.01),nested_dist, "Nested"), use.names = F))
colnames(dist_table) = c("Population_frequency", "Number_of_peptides", "HLA")
ggplot(data = dist_table)+
  geom_point(aes(x=Population_frequency, y=Number_of_peptides, color=HLA))+
  scale_color_manual(values = c("red", "blue"), name = "")+
  labs(x="Population frequency (%)", y = "Number of peptides") +
  theme(text=element_text(face="bold",size=20,colour="black"))+
  scale_y_continuous(trans = "log10", breaks = c(5,50,500,5000)) +
  theme(text=element_text(face="bold",size=20,colour="black"), 
        axis.text=element_text(size=20),
        axis.title=element_text(size=20),
        plot.title = element_text(size=20))
...
mean(unlist(lapply(strsplit(Net_BA_cast$Binding_haplotype,","), length)))
mean(unlist(lapply(strsplit(Netii_BA_cast$Binding_haplotype,","), length)))
...
prot_dist_predicted = data.table(prots, counts_I, counts_II)#, counts_coep)
colnames(prot_dist_predicted) = c("Protein", "HLA-I", "HLA-II")#, "Nested ligands")
...
prot_dist_iedb = data.table(prots, counts_iedb_I, counts_iedb_II)
colnames(prot_dist_iedb)[1] = "Protein"

#Plot as stacked barplot
prot_dist_iedb_bar = melt.data.table(prot_dist_iedb, measure.vars = c(2,3), id.vars = 1)
colnames(prot_dist_iedb_bar) = c("Protein", "Class", "Count")

prot_dist_iedb_bar$Class = as.character(prot_dist_iedb_bar$Class)

prot_dist_iedb_bar$Class[which(prot_dist_iedb_bar$Class == "counts_iedb_I")] = "HLA-I"
prot_dist_iedb_bar$Class[which(prot_dist_iedb_bar$Class == "counts_iedb_II")] = "HLA-II"

top_right = ggplot(prot_dist_iedb_bar, aes(x=Class, y=Count, fill=Protein)) +
  geom_bar(position = "stack",stat="identity", width=1, show.legend = T, size=1) +
  labs(title="IEDB ligands")+
  scale_fill_manual(values=c(viridis(10)))+
  #  geom_label_repel(aes(y = ypos_co, label = Protein), color = "white", size=8, nudge_x = .65,
  #                   box.padding   = 0.35, 
  #                   point.padding = 0.5,
...
#Plot out Fig3 top as barplots
prot_dist_pred_bar$Source = "Predicted"
prot_dist_iedb_bar$Source = "IEDB"
prot_dist_bar_all = rbind(prot_dist_iedb_bar, prot_dist_pred_bar)
prot_dist_bar_all$Protein = plyr::mapvalues(prot_dist_bar_all$Protein, 
          from = levels(prot_dist_bar_all$Protein), 
          to = str_replace(levels(prot_dist_bar_all$Protein), "orf1ab", "ORF1ab"))

#Derive %
prot_dist_bar_all$Percentage = NA
for(c in c("HLA-I", "HLA-II")){
  for(s in c("IEDB", "Predicted")){
    for(n in 1:nrow(prot_dist_bar_all)){
      if((prot_dist_bar_all$Class[n] == c) & (prot_dist_bar_all$Source[n] == s)){
        prot_dist_bar_all$Percentage[n] = 
          100*prot_dist_bar_all$Count[n]/sum(prot_dist_bar_all$Count[which((prot_dist_bar_all$Class == c) & (prot_dist_bar_all$Source == s))])
      }
    }
...
Fig3_I = ggplot(prot_dist_bar_all[which(prot_dist_bar_all$Class == "HLA-I")], aes(x=Source, y=Percentage, fill=Protein)) +
  geom_bar(position = "stack",stat="identity", width=1, show.legend = T, size=1) +
  labs(title="HLA-I ligands")+
  scale_fill_manual(values=c(viridis(10)))+
  scale_x_discrete(labels = c(paste0("IEDB\n(n=", sum(prot_dist_bar_all$Count[which((prot_dist_bar_all$Class == "HLA-I") & (prot_dist_bar_all$Source == "IEDB"))]), ")"),
                            paste0("Predicted\n(n=", sum(prot_dist_bar_all$Count[which((prot_dist_bar_all$Class == "HLA-I") & (prot_dist_bar_all$Source == "Predicted"))]), ")"))) +
  #  geom_label_repel(aes(y = ypos_co, label = Protein), color = "white", size=8, nudge_x = .65,
  #                   box.padding   = 0.35, 
  #                   point.padding = 0.5,
  #                   segment.color = 'black',
...
Fig3_II = ggplot(prot_dist_bar_all[which(prot_dist_bar_all$Class == "HLA-II")], aes(x=Source, y=Percentage, fill=Protein)) +
  geom_bar(position = "stack",stat="identity", width=1, show.legend = T, size=1) +
  labs(title="HLA-II ligands")+
  scale_fill_manual(values=c(viridis(10)))+
  scale_x_discrete(labels = c(paste0("IEDB\n(n=", sum(prot_dist_bar_all$Count[which((prot_dist_bar_all$Class == "HLA-II") & (prot_dist_bar_all$Source == "IEDB"))]), ")"),
                              paste0("Predicted\n(n=", sum(prot_dist_bar_all$Count[which((prot_dist_bar_all$Class == "HLA-II") & (prot_dist_bar_all$Source == "Predicted"))]), ")"))) +
  #  geom_label_repel(aes(y = ypos_co, label = Protein), color = "white", size=8, nudge_x = .65,
  #                   box.padding   = 0.35, 
  #                   point.padding = 0.5,
  #                   segment.color = 'black',
...
#Correct nested epitope set to account for allele coverage from IEDB set
...
bar_chart = function(mat, mat2, matco, size){
  ...
  #Without nested  
    annotate("label", y=c(.15,.85), x= 1.5, label = c(dat_text$V1, dat_text$V3), color = c("black", "black"), 
...
t3 = bar_chart(filt_I, filt_II, filt_co, 18)
...
merge_tab$best_overlap = 100* merge_tab$best_overlap

Master_tab$Group = str_replace_all(Master_tab$Group, "HLA", "HLA-")

####Make plot
y_ll=-80  #Lower limit of y axis
...
centipede = 
  ...
  geom_segment(aes(x=Start, xend=Start, y=ifelse(Master_tab$Group=="HLA-I",10,-10), yend=Immunogenic_frequency, color = Group), 
  ...
  scale_fill_manual(values = c(alpha("red",.25), alpha("blue",.25)), name="HLA")+
  ...
  geom_rect(aes(xmin=7096, xmax=8368, ymin=(y_ll), ymax=(y_ll-17)), fill=viridis(10)[2], color="black", size=0.1) +
  geom_text(aes(x=7096+(8368-7096)/2, y=(y_ll-22), label="S", angle=0), size=10, color = "black") + 
  geom_text(aes(x=7096+200, y=(y_ll), label="-200", angle=-90), size=6, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=7096+500, y=(y_ll), label="-500", angle=-90), size=6, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=7096+800, y=(y_ll), label="-800", angle=-90), size=6, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=7096+1100, y=(y_ll), label="-1100", angle=-90), size=6, color = "white",  fontface = "plain", hjust=0) +
  
  geom_rect(aes(xmin=8719, xmax=8940, ymin=(y_ll-0), ymax=(y_ll-17)), fill=viridis(10)[5], color="black", size=0.1) +
  geom_text(aes(x=8719+(8940-8719)/2, y=(y_ll-22)), label="M", angle=0, size=10, color = "black") +
  geom_text(aes(x=8719+50, y=(y_ll), label="-50", angle=-90), size=6, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=8719+100, y=(y_ll), label="-100", angle=-90), size=6, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=8719+150, y=(y_ll), label="-150", angle=-90), size=6, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=8719+200, y=(y_ll), label="-200", angle=-90), size=6, color = "white",  fontface = "plain", hjust=0) +
  
  geom_rect(aes(xmin=9244, xmax=9662, ymin=(y_ll-0), ymax=(y_ll-17)), fill=viridis(10)[9], color="black", size=0.1) +
  geom_text(aes(x=9244+(9662-9244)/2, y=(y_ll-22)), label="N", angle=0, size=10, color = "black") +
  geom_text(aes(x=9244+100, y=(y_ll), label="-100", angle=-90), size=6, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=9244+200, y=(y_ll), label="-200", angle=-90), size=6, color = "white",  fontface = "plain", hjust=0) +
  geom_text(aes(x=9244+300, y=(y_ll), label="-300", angle=-90), size=6, color = "white",  fontface = "plain", hjust=0) +
  theme(text=element_text(face="bold",size=20,colour="black"), 
        axis.text=element_text(size=20),
        axis.title=element_text(size=20),
        plot.title = element_text(size=20))
...
S1_filt = S1_filt[which(S1_filt$Predicted_tetramer == T)]
S1_filt = S1_filt[which(S1_filt$Low_polymorphism == T)]
#S1_filt = S1_filt[which(S1_filt$Protein %in% c("S", "M", "N"))]
which(S1_filt$Self_epitope == T) #No overlaps with self epitopes found!
...
for(n in 1:nrow(final_all)){  #For each entry, find the corresponding population coverage for EUR, AFA, API, HIS
  print(n)
...
#Grab these from above analysis for Figure 3 middle (funnel plot) after filtering
all_I = strsplit(paste(Window_27mer$`HLA-I_peptides`, collapse = ","), ",")
all_II = strsplit(paste(Window_27mer$`HLA-II_peptides`, collapse = ","), ",")

filt_I_sub = filt_I[which(filt_I$all_I.n. %in% all_I[[1]])]
filt_II_sub = filt_II[which(filt_II$all_II.n. %in% all_II[[1]])]
...
SD$HLA_I_epitopes = NA
SD$HLA_II_epitopes = NA
...
for(n in 1:nrow(SD)){
  ...
  I_pep = c()
  II_pep = c()
  haps_I = paste0("HLA-", SD[n,1:3])
  haps_II = c(paste0("HLA-", SD[n,5], "/", SD[5,6]), paste0("HLA-", SD[n,7]))
  
  i_count = 0
  ii_count = 0
  for(i in 1:nrow(filt_I_sub)){
    haps_I_pep = strsplit(filt_I_sub$Immunogenic_haplotypes[i], ",")
    if(any(haps_I_pep[[1]] %in% haps_I)){
      i_count = i_count + 1
    }
  }
  
  for(ii in 1:nrow(filt_II_sub)){
    haps_II_pep = strsplit(filt_II_sub$Immunogenic_haplotypes[ii], ",")
    if(any(haps_II_pep[[1]] %in% haps_II)){
      ii_count = ii_count + 1
    }
  }
  
  SD$HLA_I_epitopes[n] = i_count
  SD$HLA_II_epitopes[n] = ii_count
}

SD$Frequency_norm = round(SD$Frequency/min(SD$Frequency))

#Generate list of #HLA epitopes per haplotype
i_list = c()
ii_list = c()
for(n in 1:nrow(SD)){
  i_list = c(i_list, rep(SD$HLA_I_epitopes[n], SD$Frequency_norm[n]))
  ii_list = c(ii_list, rep(SD$HLA_II_epitopes[n], SD$Frequency_norm[n]))
}

hla_tab = rbind(cbind(as.data.table(i_list), "HLA-I"),
      cbind(as.data.table(ii_list), "HLA-II"), use.names=F)
colnames(hla_tab) = c("Number of peptides", "HLA")

(length(which(i_list == 0))/length(i_list))^2 #Squared likelihood of having 0 HLA-I epitopes (estimated population likelihood)
(length(which(ii_list == 0))/length(ii_list))^2 #Squared likelihood of having 0 HLA-II epitopes (estimated population likelihood)

ggplot() +
  geom_histogram(data = hla_tab, aes(x = `Number of peptides`, y = 100*2*(..count..)/sum(..count..), fill = HLA), position = "dodge", bins = 55) +
  scale_x_continuous(breaks = seq(0,55,5)) +
  ylab("Genetic proportion (%)") +
  theme(text=element_text(face="bold",size=20,colour="black")) 
